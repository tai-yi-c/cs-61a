...
(define (remain expr) (cdr (cdr expr)))
;(cons + (cons (derive (first-operand expr) var) (derive-sum (remain expr) var)))
...
(define (make-exp base exponent)
  ...
  (cond ((or (=number? base 1) (=number? exponent 0)) 1)
        ...
        ((=number? base 0) 0)
        ((and (number? base) (number? exponent)) (calc-exp base exponent))
...
(define (derive-exp exp var)
  ...
  (begin
      ...
      (define left-mul (make-exp f (- g 1)))
      (make-product g left-mul)
      )
)
