...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        assert len(opponent.hand) >= 3, "opponent should have at least 3 cards!"
        assert len(opponent.deck.cards) >= 3, "opponent's deck should have at least 3 cards!"
        #Uncomment the line below when you've finished implementing this method!
        print('{} discarded and re-drew 3 cards!'.format(opponent.name))
        # opponent discard first 3 cards and re-draw 3 cards from deck
        for _ in range(3):
            opponent.play(0)
            opponent.draw()
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        # remove cards from opponent.deck.cards
        j = 0
        while j < len(opponent.deck.cards):
            cur_defense, cur_attack = opponent.deck.cards[j].defense, opponent.deck.cards[j].attack
            remove_flag = False
            if cur_attack == other_card.attack and cur_defense == other_card.defense:
                opponent.deck.cards.pop(j)
                remove_flag = True
            if not remove_flag:
                j += 1
