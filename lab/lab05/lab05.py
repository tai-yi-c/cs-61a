...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    the_city = make_city("this", lat, lon)
    dis1 = distance(the_city, city_a)
    dis2 = distance(the_city, city_b)
    if dis1 < dis2:
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        sprout = [tree(leave) for leave in leaves]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        more = abs(len(branches(t1)) - len(branches(t2)))
        if len(branches(t1)) > len(branches(t2)):
            new_branches += branches(t1)[-more:]
        elif len(branches(t1)) < len(branches(t2)):
            new_branches += branches(t2)[-more:]
...
def prune_binary(t, nums):
    """
    >>> t = tree('1', [tree('0',[tree('0'), tree('1')]), tree('1', [tree('1')])])
    >>> pb =prune_binary(t, ['01', '110', '100'])
    >>> bool(pb) == False
    False
    >>>print_tree(pb)
    ""
    """
    if label(t) not in [binary[0] for binary in nums]:
        if len(nums) == 0:
            return t
        print("<<<", label(t))
        print("<<<", [binary[0] for binary in nums])
        return None
    else:
        next_valid_nums = [binary[1:] for binary in nums if len(binary[1:]) > 0]
        new_branches = []
        for b in branches(t):
...
def build_successors_table(tokens):
    ...
    for word in tokens:
        if prev not in table:
            table[prev] = [word]
        if word not in table[prev]: table[prev].append(word)
        prev = word
    return table
...
def construct_sent(word, table):
    ...
    result = ''
    while word not in ['.', '!', '?']:
        next_token = random.choice(table[word])
        result = result + " " + word
        word = next_token
