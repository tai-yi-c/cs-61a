...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        oper_fn = self.operator.eval(env)
        arguments = []
        for operand in self.operands:
            arguments.append(operand.eval(env))
        return oper_fn.apply(arguments)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        cur_frame = self.parent.copy()
        for formal, arg in zip(self.parameters, arguments):
            cur_frame[formal] = arg
        return self.body.eval(cur_frame)
