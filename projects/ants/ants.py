...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            exist_ant = place.ant
            new_ant = self
            if isinstance(exist_ant, ContainerAnt) and exist_ant.can_contain(new_ant):
                exist_ant.contained_ant = new_ant
            elif isinstance(new_ant, ContainerAnt) and new_ant.can_contain(exist_ant):
                new_ant.contained_ant = exist_ant
                place.ant = new_ant
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        while distance != self.min_range and (check_place is not beehive):
            check_place = check_place.entrance
            distance += 1


        while distance <= self.max_range and (check_place is not beehive):
            if check_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in cur_place.bees[:]:
            ...
            # if a insect is removed, its place will be set as None.
            if self.armor <= 0 and bee.place:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        # END Problem 6
        if self.digesting > 0:
            # if it is digesting then digest
            self.digesting -= 1
        else:
            # otherwise choose a random bees in its place to eat
            dying_bees = rANTdom_else_none(self.place.bees)
            if dying_bees:
                dying_bees.reduce_armor(dying_bees.armor)
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.the_real_queue:
            self.the_real_queue = True
            QueenAnt.the_real_queue = False
        else:
            self.the_real_queue = False
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        # END Problem EC
        if self.the_real_queue == False:
            self.reduce_armor(self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            cur_place = self.place.exit
            while cur_place:
                if cur_place.ant and cur_place.ant.cheered == False:
                    cur_place.ant.damage *= 2
                    cur_place.ant.cheered = True
    ...
    def reduce_armor(self, amount):
        ...
        # END Problem EC
        Ant.reduce_armor(self, amount)
        if self.armor <= 0 and self.the_real_queue:
            bees_win()

    def remove_from(self, place):
        if self.the_real_queue:
            return
        else:
...
def apply_status(status, bee, length):
    ...
    "*** YOUR CODE HERE ***"
    """
    status: new action method, maybe it's not the initial action
    apply_status: it's a rebound_action function

    problem: how to add time message to the method? where's the time message???
    """
    initial_action = bee.action
    initial_direction = bee.direction
    def buff_action_with_time(gamestate):
        """a new action method with time judge
        """
        nonlocal length
        if length > 0:
            buff_action = status(initial_action, bee)
            buff_action(gamestate)
            length -= 1
        else:
            bee.direction = initial_direction
            initial_action(gamestate)

    # rebound bee.action to new defined buff_action_with_time
    bee.action = buff_action_with_time
    # END Problem Optional 4
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        cur_place = self.place
        cur_distance = 0
        insects_with_position = {}
        while cur_place != beehive:
            for bee in cur_place.bees:
                insects_with_position[bee] = cur_distance
            if cur_place.ant and (cur_place.ant is not self):
                insects_with_position[cur_place.ant] = cur_distance

            cur_distance += 1
            cur_place = cur_place.entrance

        return insects_with_position
        # END Problem Optional 5

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        cur_damage = self.damage - (self.insects_shot * self.decrease_number) - self.decreasing_rate * distance
        return cur_damage
        # END Problem Optional 5
