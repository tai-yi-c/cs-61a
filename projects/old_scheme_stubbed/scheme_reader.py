...
def scheme_read(src):
    ...
    """
    first judge the legitimacy of src,then transfer
    the src from Buffer to expresion of python type
    :param:
        src: Buffer
    :returns:
        Pair or built-in python type: string, int/float, True/False, nil
    """
    if not isinstance(src, Buffer):
        raise TypeError("scheme_read input src should belong to Buffer")
    else:
        signal = src.current()
        src.pop_first()
        # expression of combination start with "("
        if signal == "(":
            operator = src.current()
            if operator is None:
                raise SyntaxError("wrong expression")
            src.pop_first()
            return Pair(operator, read_tail(src))
        elif src.current() is None:
            # single expression
...
def read_tail(src):
    ...
    Pair(2, Pair(3, nil))
    """
    # assume all input is all right on syntax!!!!!!!!
    try:
        if src.current() is None:
            raise SyntaxError('unexpected end of file')
        # BEGIN PROBLEM 1
        # "*** YOUR CODE HERE ***"
        signal = src.pop_first()
        if signal == ")":
            ...
            cur = src.pop_first()
            pair1 = Pair(cur, read_tail(src))
            pair2 = read_tail(src)
            return Pair(pair1, pair2)
        else:
            return Pair(signal, read_tail(src))
