...
def swine_align(player_score, opponent_score):
    ...
    if opponent_score == 0:
        return False
    mygcd = gcd(player_score, opponent_score)
    if mygcd >= 10:
        return True
    else:
        return False
...
def play_game(who, strategy, score, opponent, dice, goal):
    """player: who, will use strategy to determine num_rolls,and
    ...
    """
    assert score < goal, "0:it should finish before this turn"
    assert opponent < goal, "1:it should finish before this turn"

    should_change = lambda x, y: (not swine_align(x, y)) and (not pig_pass(x, y))

    num_rolls = strategy(score, opponent)
    plus_score = take_turn(num_rolls, opponent, dice)
    score += plus_score
    if should_change(score, opponent):
        who = 1 - who
    return who, score
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        if who == 0:
            who, score0 = play_game(who, strategy0, score0, score1, dice, goal)
        elif who == 1:
            who, score1 = play_game(who, strategy1, score1, score0, dice, goal)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # END PROBLEM 7
    def say(score0, score1):
        this_score = score0 if who == 0 else score1
        add = this_score - last_score
        high = running_high
        if add > running_high:
            high = add
            print("{} point(s)! The most yet for Player {}".format(add, who))
        return announce_highest(who, this_score, high)
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def run_original_and_return_average_results(*args):
        i = 0
        sums = 0
        while i < trials_count:
            ...
            sums += result
            i += 1
        return sums / trials_count * 1.0
    return run_original_and_return_average_results
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_rolls = 0
    high_results = 0
    experiment = make_averaged(roll_dice, trials_count)
    for i in range(1,11):
        cur = experiment(i,dice)
        if cur > high_results:
            high_results = cur
            num_rolls = i
    return num_rolls
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    prob_get_score = free_bacon(opponent_score)
    if prob_get_score >= cutoff or extra_turn(score + prob_get_score, opponent_score):
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    num_rolls = 10
    cutoff = abs(score - opponent_score) + 1
